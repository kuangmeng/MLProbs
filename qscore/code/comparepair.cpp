...
const MSA *g_ptrmsaTest = 0;
const MSA *g_ptrmsaRef = 0;
...
void ComparePair(const MSA &msaTest, unsigned uTestSeqIndexA,
  ...
  unsigned uRefSeqIndexB, double *ptrdSP, double *ptrdPS, double *ptrdCS)
	...
	g_ptrmsaTest = &msaTest;
	g_ptrmsaRef = &msaRef;
	g_TestSeqIndexA = uTestSeqIndexA;
	g_TestSeqIndexB = uTestSeqIndexB;
	g_RefSeqIndexA = uRefSeqIndexA;
	g_RefSeqIndexB = uRefSeqIndexB;

	if (iLengthA != iLengthAr)
		Quit("Seq %s has %u letters in test, %u letters in ref",
