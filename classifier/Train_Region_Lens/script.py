...
def ReadEXCEL(excel_file):
    ...
    for j in range(nrows):
        ...
        if row[0][0:5] == "sabre" or row[0][0:2] == "ox":
            if float(row[1]) >= float(row[2]) and float(row[1]) >= float(row[3]) and float(row[1]) >= float(row[4]):
                ret_matrix.append([row[0], 0])
            elif float(row[2]) >= float(row[3]) and float(row[2]) >= float(row[4]):
                ret_matrix.append([row[0], 1])
            elif float(row[4]) >= float(row[3]):
                ret_matrix.append([row[0], 3])
            else:
                ret_matrix.append([row[0], 2])
        elif row[0][0:3] == "oxx":
            if float(row[3]) >= float(row[1]) and float(row[3]) >= float(row[2]) and float(row[3]) >= float(row[4]):
                ret_matrix.append([row[0], 2])
            elif float(row[2]) >= float(row[3]) and float(row[2]) >= float(row[4]):
                ret_matrix.append([row[0], 1])
                ...
                ret_matrix.append([row[0], 3])
        elif row[0][0:5] == "bali3" or row[0][0:6] == "prefab":
            if float(row[4]) >= float(row[2]) and float(row[4]) >= float(row[1]) and float(row[4]) >= float(row[3]):
                ret_matrix.append([row[0], 3])
            elif float(row[3]) >= float(row[1]) and float(row[3]) >= float(row[2]):
                ret_matrix.append([row[0], 2])
