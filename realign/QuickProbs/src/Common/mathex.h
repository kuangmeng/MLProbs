...
inline U convertUp(T x) { return (T)x; }


template <>
inline float convertUp<double, float>(double x) 
{
	float y = (float)x;
	if (y < x) {
	#ifdef WIN32
		y = _nextafterf(y, std::numeric_limits<float>::infinity());
	#else
		y = std::nextafterf(y, std::numeric_limits<float>::infinity());
	#endif
	}
	return y;
}
