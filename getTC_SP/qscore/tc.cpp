...
void SeqsDiffer(MSA &msaTest, unsigned uTestSeqIndex1, MSA &/*msaRef*/,
  unsigned /*uRefSeqIndex1*/, unsigned /*uRefColIndex*/)
	{
	Quit("Test & ref sequences differ, label=%s", msaTest.GetSeqName(uTestSeqIndex1));
	}

bool IsAlignedColWithGaps(const MSA &msa, unsigned uColIndex)
	{
...
int IsAlignedCol(const MSA &msa, unsigned uColIndex)
	{
	const unsigned uSeqCount = msa.GetSeqCount();
	for (unsigned uSeqIndex = 0; uSeqIndex < uSeqCount; ++uSeqIndex)
		{
